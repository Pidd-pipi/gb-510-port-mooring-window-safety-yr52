...
type WeatherWindowRepository interface {
	...
	GetByCodeForUpdate(context.Context, *gorm.DB, string) (model.WeatherWindow, error)
...
// GetByCodeForUpdate loads the related 风浪窗口 with a row lock for the duration
// of an approval transaction, so a window cannot flip safe -> restricted while
// the occupancy is being acquired.
func (r *weatherWindowRepository) GetByCodeForUpdate(ctx context.Context, tx *gorm.DB, code string) (model.WeatherWindow, error) {
	var item model.WeatherWindow
	query := tx.WithContext(ctx).Where("code = ?", code)
