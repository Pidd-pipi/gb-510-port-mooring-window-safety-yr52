...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	berthOccupancyRepository := repository.NewBerthOccupancyRepository(db)
	vesselCallService := service.NewVesselCallService(vesselCallRepository, securityService)
	mooringPlanService := service.NewMooringPlanService(db, mooringPlanRepository, berthOccupancyRepository, weatherWindowRepository, securityService)
